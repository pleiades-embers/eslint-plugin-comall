...
function checkExpressionInfo(node) {
  ...
  if (
    ["Identifier", "MemberExpression", "ThisExpression"].includes(node.type)
  ) {
    return null;
  }
...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        "除了基本数据类型、对象成员引用、变量，其他表达式不允许传递给jsx中的组件属性上 ",
  ...
  create(context) {
    return {
      JSXAttribute(node) {
        const jsxAttributeName = node.name && node.name.name;
        ...
        if (
          valueExpression.type === "TemplateLiteral" &&
          valueExpression.expressions.length
        ) {
          return;
        }
